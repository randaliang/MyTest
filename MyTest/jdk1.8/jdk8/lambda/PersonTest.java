...
public class PersonTest {
	static List<Person> list = Arrays.asList(new Person("Yixing", "zhao", 25), new Person("Yanggui", "Li", 30),
	...
	public static void main(String args[]) {
		...
		Optional<Person> o = list.stream().max(new Comparator<Person>() {
			@Override
		...
		list.stream().filter(p -> p.getLastName().length() > 1).forEach(p -> System.out.println(p.getLastName()));

		list.stream().filter(p -> p.getLastName().length() > 1).forEach(System.out::println);
